...
*El siguiente código se encarga de extraer la información de las llaves que representan características del clima. Y luego busca información de las llaves internas (Es decir los números que representan los días de la semana), con el fin de encontrar datos relevantes.*
```python
import json
...
print(data.get('alertAlertas'))
print(data.get('alertPrecip'))
print(data.get('alertTmpMax'))
print(data.get('alertTmpMin'))
print(data.get('alertVelViento'))

print(list(data.keys()))
print()
print("Alerta de precipitaciones - día 0: ")
tiempo_0 = data.get('dt',{}).get('0')
tiempo_format_0=datetime.utcfromtimestamp(tiempo_0).strftime('%Y-%m-%d %H:%M:%S')
print(f"Fecha: {tiempo_format_0}")
print(f"La humedad es {data.get('humidity', {}).get('0')}")
print(f"La presión es {data.get('pressure', {}).get('0')}")
print(f"El punto de rocío es {data.get('dew_point',{}).get('0')}")
print(f"El clima es {data.get('weather',{}).get('0')}")
print(f"Las nubes son {data.get('clouds',{}).get('0')}")
print(f"La probabilidad de precipitación es {data.get('pop',{}).get('0')}")
print(f"La precipitación es {data.get('prcp', {}).get('0')}")
print()
print()
print("Alerta de temperatura máxima - día 5: ")
tiempo_5 = data.get('dt',{}).get('5')
tiempo_format_5=datetime.utcfromtimestamp(tiempo_5).strftime('%Y-%m-%d %H:%M:%S')
print(f"Fecha: {tiempo_format_5}")
print(f"El amanecer  es {data.get('sunrise', {}).get('5')}")
print(f"El atardecer es {data.get('sunset', {}).get('5')}")
print(f"El índice UV es {data.get('uvi',{}).get('5')}")
print(f"El clima es {data.get('weather',{}).get('5')}")
print(f"La temperatura máxima es {data.get('tmpMax',{}).get('5')}")
print()
print()
print("Alerta de temperatura mínima - día 1: ")
tiempo_1 = data.get('dt',{}).get('1')
tiempo_format_1=datetime.utcfromtimestamp(tiempo_1).strftime('%Y-%m-%d %H:%M:%S')
print(f"Fecha: {tiempo_format_1}")
print(f"El amanecer  es {data.get('sunrise', {}).get('1')}")
print(f"El atardecer es {data.get('sunset', {}).get('1')}")
print(f"El índice UV es {data.get('uvi',{}).get('1')}")
print(f"El clima es {data.get('weather',{}).get('1')}")
print(f"La temperatura mínima es {data.get('tmpMin',{}).get('1')}")
print()
print()
print("Alerta de vientos - día 2: ")
tiempo_2 = data.get('dt',{}).get('2')
tiempo_format_2=datetime.utcfromtimestamp(tiempo_2).strftime('%Y-%m-%d %H:%M:%S')
print(f"Fecha: {tiempo_format_2}")
print(f"La velocidad del viento es {data.get('velViento', {}).get('2')}")
...
```python
import json
import requests      #Importante para conectarse a API'S

url_1 = 'https://catfact.ninja/fact'

response_1 = requests.get(url_1)    #Carga el link
cats = response_1.json()            #Lo almacena como archivo json
print(cats)                         #Imprime el diccionario        
...
print(genderize)
print(list(genderize.items()))     #Crea tupla
```
